...
import json
import re
from common import Account, memoize, profile
...
ARN_REGEX = r'^arn:aws:(?P<service_name>.*?):(?P<region>.*?):(?P<account_id>.*?):'
...
class PublicEndpointReport:
        ...
        for resource in resources:
            if self.does_resource_exist(resource):
                if resource['ResourceType'] == 'AWS::SQS::Queue':
                    # Fix difference between SQS URLs
                    resource['PhysicalResourceId'] = re.sub(
                        r'sqs\.(.*)\.amazonaws',
                        r'\1.queue.amazonaws',
    ...
    def does_condition_allow_public_access(self, condition):
        ...
        for operator in operators:
            if operator in condition:
                for key, values in condition[operator].items():
                    key = key.lower()
                    if key == 'aws:sourceowner':
                        account_ids = values if type(values) is list else [values]

                        for id in account_ids:
                            print('Warning: allowing access from {}'.format(id))
                        ...
                        org_ids = values if type(values) is list else [values]

                        for org_id in org_ids:
                            print('Warning: allowing access from {}'.format(org_id))
    ...
    def get_sqs_tags(self, queue_url, region):
        client = self.account.get_client('sqs', region)
        response = client.list_queue_tags(
            QueueUrl=queue_url
        )

        # Switch to conventional tag format
        return [
            {
                'Key': key,
                'Value': value,
    ...
    def get_public_sqs_endpoint(self, queue_url, region):
        tags = self.get_sqs_tags(queue_url, region)
    ...
    def get_all_public_endpoints(self):
        ...
        for endpoint in endpoints:
            ...
            # Look up the resource in the list of CloudFormation Resources
            if resource_type in self.resources \
               and resource_id in self.resources[resource_type]:
                stack_information = self.resources[resource_type][resource_id]

                # Only overwrite keys that are empty right now
                for key, value in stack_information.items():
                    if endpoint[key] == '':
                        endpoint[key] = value
...
[ebin.davis@ip-10-200-4-120 scripts]$
